#!/bin/bash
# Build and push Docker image to ECR (no cache for fresh builds)

set -e

echo "🐳 Building and Pushing Iris Flow Docker Image (No Cache)"
echo "==========================================================="
...
echo "🔨 Building Docker image for linux/amd64 (no cache)..."
cd "$(dirname "$0")"
docker build --no-cache --platform linux/amd64 -f docker/Dockerfile -t iris-flow-generator .
